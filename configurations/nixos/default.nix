inputs: let
  ...
  in {
    DesktopFull = mkNixOSConfig username { 
      custom.apps = [ "android" "vscode" "spotify" "htop" "uv" "cursor" "tex-live" "docker" "zsh" "llvm" "mlir" "life-tracker" "ffmpeg" ]; 
    };
    DesktopStandard = mkNixOSConfig username { 
      custom.apps = [ "android" "cursor" "spotify" "vscode" "uv" "rustup" "chrome" "dotnet" "llvm" "mlir" "life-tracker" "ffmpeg" ]; 
    };
